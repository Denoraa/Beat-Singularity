...
2. 
I updated my break-down by adding a state machine system for the rhythm gameplay flow. The level now moves through LoadChart, Ready, Playing, and End states. LoadChart imports the CSV chart data, Ready waits for the player to begin, Playing activates music, note spawning, hit detection, and combo tracking, and End happens when the song finishes. Each note also has its own smaller states: Spawned, Active, Hit, or Missed.
